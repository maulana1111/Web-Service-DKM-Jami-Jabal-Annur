			...
			if(move_uploaded_file($_FILES['imageupload']['tmp_name'], $destination))
			{
				...
				if($cek > 0)
				{
					...
					if($now_date >= $batas_waktu)
					{
						...
						mysqli_query($conn, "UPDATE anggota_janaiz SET id_transaksi = '$ndetail_batas' WHERE id = '$id_anggota' ");
				...
				$detTra = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM transaksi_janaiz WHERE id_anggota = '$id_anggota' ORDER BY id DESC LIMIT 1"));
				$id_transaksi = $detTra['id'];

				mysqli_query($conn, "UPDATE anggota_janaiz SET id_transaksi = '$id_transaksi' WHERE id = '$id_anggota'");
